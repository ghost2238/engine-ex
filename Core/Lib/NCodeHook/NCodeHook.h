...
public:
	...
	uintptr_t getNextFreeTrampoline();
	...
	//template <typename U> U getOriginalFuncFromTrampoline(U trampolineFunc);
	template <typename U> bool removeHook(U address);
	
	std::map<uintptr_t, NCodeHookItem> getHookedFunctions() { return hookedFunctions_; }
